...
def overview():
    ...
    trace1 = go.Pie(labels=['Positive', 'Negative', 'Neutral', 'Mixed'], values=[ind_sent[0],ind_sent[1],ind_sent[2],ind_sent[3]])
    data1 = [trace1]
    ...
    return render_template('overview.html',langJSON=langJSON,country_sent_ind=country_sent_ind,country_sent_us=country_sent_us,country_sent_mex=country_sent_mex,dates=lab,val=val)
...
def tweetsearch():
    ...
    query_sent='http://' + AWS_IP + ':' + AWS_PORT + '/solr/' + SOLR_CORE_NAME + '/select?fl=id%20score%20tweet_text%20sentiment%20sentiment_score&q=text_en%3A(' + encoded_query + ')%20or%20text_hi%3A(' + encoded_query + ')%20or%20text_es%3A(' + encoded_query + ')&rows=58000'
    ...
    data1=requests.get(query_sent).json()
    lang_json=requests.get(query_calc).json()
    total_lang=0
    total_hi=0
    total_en=0
    total_es=0
    for i in lang_json['response']['docs']:
        ...
        if(tweet_lang=='hi'):
            ...
            total_es+=1
    hi_per=((total_hi/total_lang)/100)
    en_per = ((total_en / total_lang) / 100)
    es_per = ((total_es / total_lang) / 100)
    trace2 = go.Pie(labels=['English','Spanish','Hindi'], values=[en_per, es_per, hi_per])
    data3 = [trace2]
    ...
    return render_template('basic_page.html', tweet_search=docs, tweet_news=news_docs,graphJSON=graphJSON,langJSON=langJSON)
